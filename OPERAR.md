...
git log --oneline -5
curl -s \
  "https://api.github.com/repos/mikebalcazar/docs101/actions/runs?per_page=3" \
...
```bash
curl -s -X POST \
  https://api.github.com/repos/mikebalcazar/docs101/pulls \
  -d '{"title":"…","head":"claude/…","base":"main","body":"…"}'
# y con el número que devuelve:
curl -s -X PUT \
  https://api.github.com/repos/mikebalcazar/docs101/pulls/<N>/merge \
...
curl -s \
  "https://api.github.com/repos/mikebalcazar/docs101/commits/<SHA>/comments" \
  | python3 -c "import json,sys; [print(c['body']) for c in json.load(sys.stdin)]"

# y si algo falló, en qué trabajo fue
curl -s \
  "https://api.github.com/repos/mikebalcazar/docs101/actions/runs/<ID>/jobs" \
...
```bash
curl -s \
  "https://api.github.com/repos/mikebalcazar/docs101/actions/permissions/workflow"
```
...
```bash
curl -s -X POST \
  "https://api.github.com/repos/mikebalcazar/docs101/actions/workflows/verificar.yml/dispatches" \
